...
let baseCurrency, currenciesExchangeRates, currenciesActiveArray, currenciesPriorityDefault, headerRow, valueRow, statusText, readyMessage, timestampTime, timestampParagraph, apiKey, apiUrl;
  ...
  for (let currency of currenciesActiveArray) {
    ...
    valuesHTMLString += `
        <td class="currencyConverter__tableCell js-columnValue${columnIndex}">
          Kwota ${columnIndex}
          <input type="number" value="${convertToAnyCurrency(startingAmount, currenciesActiveArray[0], currenciesActiveArray[columnIndex - 1]).toFixed(
            2,
...
function recreateTableEventListeners() {
  for (let columnIndex = 1; columnIndex <= currenciesActiveArray.length; columnIndex++) {
    document.querySelector(".js-amount" + columnIndex).addEventListener("input", () => {
      recalculateForeignValues(eval(columnIndex));
    });
    document.querySelector(".js-select" + columnIndex).addEventListener("input", () => {
      handleCurrencyChange(eval(columnIndex));
    });
    document.querySelector(".js-minus" + columnIndex).addEventListener("click", () => {
      removeColumn(eval(columnIndex));
    });
    document.querySelector(".js-plus" + columnIndex).addEventListener("click", () => {
      addColumn(eval(columnIndex));
    });
  }
}
...
function recalculateForeignValues(enteredFieldIndex) {
  ...
  for (let inputIndex = 1; inputIndex <= currenciesActiveArray.length; inputIndex++) {
    if (inputIndex !== enteredFieldIndex) {
...
function checkIncomingExchangeRates(data) {
  ...
  if (currencyDataArray === false) {
    return statusMessage("BŁĄD: W odpowiedzi nie otrzymaliśmy prawidłowego JSON!", true);
  }
  if (!currencyDataArray.rates) {
    return statusMessage("BŁĄD: W odpowiedzi nie otrzymaliśmy wymaganych danych!", true);
  }
  ...
  timestampTime.innerHTML = `<time datetime="${currencyDataArrayTimestamp.toISOString()}">${currencyDataArrayTimestamp.toLocaleString()}</time>. ` + `Łącznie ${Object.keys(currenciesExchangeRates).length} walut.`;
